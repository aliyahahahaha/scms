		...
		<div class="dropdown mx-3 mt-2">
			...
			<i class="fa-solid fa-bars text-primary"></i>
				...
				<div class="dropdown-menu dropdown-menu-end" aria-labelledby="orederStatistics">
							<li><?= $this->Html->link(__('<i class="fa-solid fa-plus"></i> New Activity'), ['action' => 'add'], ['class' => 'dropdown-item', 'escapeTitle' => false]) ?></li>
							</div>
		</div>
</div>
</div>
...
<tr>
                    ...
                    <th><?= $this->Paginator->sort('club_id','Club Name')?></th>
                    <th><?= $this->Paginator->sort('name','Activities')?></th>
                    <th><?= $this->Paginator->sort('date', 'Date') ?></th> 
					<th><?= $this->Paginator->sort('approvalby','Approval By') ?></th>
					<th><?= $this->Paginator->sort('approvaldate','Approval Date')?></th>
                    <th><?= $this->Paginator->sort('approvalstatus', 'Status') ?></th>
            ...
            <tbody>
				...
				<td><?php echo $counter++ ?></td>
                    <td><?= ($activity->user->fullname) ?><br />
					<?= ($activity->faculty->name) ?><br />
					<?= ($activity->program->name) ?><br />
					<?= ($activity->semester->code) ?><br />
				</td>
					...
					<td><?= h($activity->approvalby) ?></td>

					<td><?= h($activity->approvaldate) ?></td>

                    <td>
						<?php if ($activity->approvalstatus == 0) {
					...
					<td class="actions text-center">
						<div class="btn-group shadow" role="group" aria-label="Basic example">
							<?= $this->Html->link(__('<i class="far fa-folder-open"></i>'), ['action' => 'view', $activity->id], ['class' => 'btn btn-outline-primary btn-xs', 'escapeTitle' => false]) ?>
							<?= $this->Html->link(__('<i class="fa-regular fa-pen-to-square"></i>'), ['action' => 'edit', $activity->id], ['class' => 'btn btn-outline-warning btn-xs', 'escapeTitle' => false]) ?>
							...
							<?= $this->Form->postLink(
								...
								[
									'confirm' => __('Are you sure you want to delete Activities: "{0}"?', $activity->id),
									'title' => __('Delete'),
			...
			<div class="row pb-3">
				...
				<div class="col-md-4">
				  <div class="stat_card card-2 bg-body-tertiary">
					<h3><?php echo $total_activities_active; ?></h3>
					<p>Active Activities</p>
				  </div>
				</div>
				<div class="col-md-4">
				  <div class="stat_card card-3 bg-body-tertiary">
					<h3><?php echo $total_activities_archived; ?></h3>
...
<div class="row">
	...
	<div class="card bg-body-tertiary border-0 shadow mb-4">
		<div class="card-body">
			<div class="card-title mb-0">Activities (Monthly)</div>
...
const monthly = new Chart(ctx, {
	...
	<div class="card bg-body-tertiary border-0 shadow mb-4">
		<div class="card-body">
		<div class="card-title mb-0">Activities by Status</div>
			<div class="tricolor_line mb-3"></div>
		<div class="chart-container" style="position: relative;">
...
const status = new Chart(ctx_2, {
    ...
    data: {
        ...
        datasets: [{
            label: '# of Activities(s)',
			data: [<?= json_encode($total_activities_active); ?>, <?= json_encode($total_activities_disabled); ?>, <?= json_encode($total_activities_archived); ?>],
            backgroundColor: [
